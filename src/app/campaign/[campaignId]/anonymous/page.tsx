...
import { addYearsToNow, calculateTotalTransactionAmountAndServiceFee, cn, phoneNumberFormater } from "@/lib/utils";
...
const FormSchema = z
  .object({
    ...
    email: z.string({ required_error: "Email wajib diisi" }).min(1, { message: "Email wajib diisi" }),
  ...
  .superRefine((data, ctx) => {
    ...
    if (data.campaign_type == 3) {
      ...
      const wakifFieldErrors: Record<string, string> = {
        ...
        ...(data?.wakif_province !== "999" ? { wakif_city: "Kota/Kabupaten harus dipilih" } : {}),
...
const AnonymousDonatePage = ({ params }: { params: { campaignId: string } }) => {
  const { currentDomain } = useFeatureFlag();
  const isAppsDomain = currentDomain?.split(".")?.[0] == "apps";
  const MIN_NOMINAL_UPDATE = params?.campaignId == "d41c661c-3968-4c6d-93b3-87b1cf2e816e" ? 1_000_000 : MINIMUM_NOMINAL;
  ...
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: {
      ...
      campaign_is_permanent:
        donationAnonymous?.campaign_is_permanent != null
          ? donationAnonymous?.campaign_is_permanent
      ...
      category_waqaf: donationAnonymous?.category_waqaf || campaign?.is_waqaf_money == 1 ? 1 : 0,
      campaign_type: campaign?.type,
      wakif_temporer_due_date: donationAnonymous?.wakif_temporer_due_date || "",

      donor_name: isAppsDomain ? "" : donationAnonymous?.donor_name || "",
      ...
      phone_number: isAppsDomain ? "" : donationAnonymous?.phone_number || "",
      ...
      wakif_identity: donationAnonymous?.wakif_identity || "",
      wakif_bank: donationAnonymous?.wakif_bank || "",
      wakif_bank_name: donationAnonymous?.wakif_bank_name || "",
      wakif_bank_account_no: donationAnonymous?.wakif_bank_account_no || "",
      wakif_bank_account_name: donationAnonymous?.wakif_bank_account_name || "",
      ...
      wakif_pray: donationAnonymous?.wakif_pray || "",
    },
  ...
  const defaultJustQris = campaign?.payment_methods?.instant_payment?.[0]?.id || "c7d3a66e-e9ca-4c7f-a020-ad75da1472b7"; // QRIS XENDIT
  const defaultJustQrisToCorporate = !!campaign?.payment_methods?.instant_payment?.[0]?.to_corporate;

  const { data: provinsi, isLoading: isLoadingProvince } = useGetProvinceList({});
  const isKantorPusat = form.watch("wakif_province") === "999";

  const { data: kota, isLoading: isLoadingCity } = useGetCityList({
    provinceId: form.watch("wakif_province") || "",
    enabled: !!form.watch("wakif_province") && !isKantorPusat,
  });
  ...
  const updateStateBeforeAction = () => {
    const payload = {
      ...
      campaign_is_permanent: donationAnonymous?.campaign_is_permanent,
  ...
  const handleSelectPaymentMethod = () => {
    ...
    router.push(`/campaign/${params?.campaignId}/anonymous/payment-method`);
  };

  const interceptLoadOptionsCorpUnitLevel1 = async (inputValue: string) => {
    const options: any[] = await loadOptionsCorpUnitLevel1(
      campaign?.corp_id,
      showInputCorpsDefault || isKantorPusat ? "" : form.watch("wakif_province") || "", // Dont have wakif types or KANTOR PUSAT dont filter province
      showInputCorpsDefault || isKantorPusat ? "" : form.watch("wakif_city") || "", // Dont have wakif types or KANTOR PUSAT dont filter city
    )(inputValue);
  ...
  const onSubmit = async (values: z.infer<typeof FormSchema>) => {
    try {
      const payload = {
        ...
        maintenance_fee: donationAnonymous?.maintenance_fee,
        payment_method_id: paymentType?.justQris ? defaultJustQris : donationAnonymous?.payment_method_id || "",
      ...
      if (
        ...
        form.watch("wakif_type") !== "Rohis Indonesia"
      ) {
      ...
      if (
        form.watch("wakif_type_has_corp_unit") &&
        !form.watch("corp_unit_lvl1_id") &&
        form.watch("wakif_type") !== "Rohis Indonesia"
      ) {
        form.setError("corp_unit_lvl1_id", { message: `${corpLevel1Nomenclature} wajib diisi` });
      ...
      if (Number.isNaN(donationAnonymous?.amount) || Number(donationAnonymous?.amount) < MIN_NOMINAL_UPDATE) {
        setErrorMinWakaf(true);
        notifyError(`Nominal wakaf minimal Rp ${MIN_NOMINAL_UPDATE}`);
        return;
      }
  ...
  const handleDonate = async () => {
    try {
      ...
      const payload = {
        ...
        wakif_name: form.getValues("wakif_name"),
        wakif_phone: form.getValues("wakif_phone") ? phoneNumberFormater(form.getValues("wakif_phone") || "") : null,
      ...
      if (donationAnonymous?.paymentMethod?.to_corporate == 1 || defaultJustQrisToCorporate) {
        ...
        .then((response) => {
          ...
          setTimeout(() => {
            if (payment?.name === "QRIS") {
              router.push(`/campaign/${params.campaignId}/anonymous/${id}?type=qris`);
            } else {
              router.push(`/campaign/${params.campaignId}/anonymous/${id}?type=va`);
            }
          }, 800);
          setLoading(false);
          setOpenIkrar(false);
  ...
  useEffect(() => {
    if (campaign?.id && paymentType?.justQris) {
      updateDonationAnonymous({ payment_method_id: defaultJustQris });
    }
  ...
  return (
    <section className="pt-16 relative layout bg-white min-h-screen">
      <Header
        inverted
        title={`Tunaikan ${textTypeCampaign}`}
        ...
        <div className={cn("border rounded-lg py-2 px-5", errorMinWakaf ? "border-danger-500" : "")}>
          ...
          {errorMinWakaf ? (
            <span className="text-xs text-danger-500">
              Nominal {textTypeCampaign} minimal Rp {MIN_NOMINAL_UPDATE}
            </span>
          ) : null}
        </div>
      </section>
                ...
                {!donationAnonymous?.paymentMethod ? (
                  ...
                  <div className="w-full inline-flex items-center justify-between pt-2">
                    <div className="inline-flex items-center space-x-2">
                      {donationAnonymous?.paymentMethod ? (
                        <img
                          src={donationAnonymous?.paymentMethod?.logo}
                          alt={donationAnonymous?.paymentMethod?.name}
                          width={50}
                          height={50}
                          fetchPriority="high"
                        />
                      ...
                      <RenderMetodePembayaran donationAnonymous={donationAnonymous} paymentType={paymentType} />
                    </div>
                    <div className="flex items-center gap-1">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input {...field} placeholder="Masukkan Email Anda" disabled={isSubmitting} />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )}
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          {...field}
                          type="number"
                          placeholder={`Masukkan No Telepon ${personCampaignLabel} (Opsional)`}
                          disabled={isSubmitting}
                        />
                      </FormControl>
                      <FormMessage />
                        ...
                        <FormItem>
                          <FormControl>
                            <ReactSelect2
                              ...
                              maxMenuHeight={150}
                              value={
                                geolocations?.cities?.find((item: any) => item?.value == form.watch("wakif_city")) ||
                                null
                              }
            ...
            {campaign?.rakornas_kemenag === 1 && (showInputCorpsDefault || form.watch("wakif_type_has_corp_unit")) ? (
              <Fragment>
                <h1 className="text-sm font-semibold my-3">
                  Informasi Institusi {form.watch("wakif_type") === "Rohis Indonesia" && "(Opsional)"}
                </h1>
                      ...
                      <FormItem>
                        <FormControl>
                          <AsyncSelect
                            ...
                            isDisabled={isLoading || (!form.watch("wakif_city") && !isKantorPusat)}
                            maxMenuHeight={150}
                            value={
                              form.watch("corp_unit_lvl1_id")
                                ? {
...
export default AnonymousDonatePage;
