...
import currencyFormater, { calculateTotalDonation, cn, phoneNumberFormater, toMoney } from "@/lib/utils";
...
import { ArrowLeftIcon, ChevronRight, ChevronRightIcon, XIcon } from "lucide-react";
...
const DEFAULT_MINIMUM_NOMINAL = 5000;
...
const Page = () => {
  ...
  const { activeProgram, programPayload } = useActiveProgram();
  ...
  const form = useForm<z.infer<typeof FormSchema>>({
    ...
    defaultValues: {
      ...
      wakif_province: donationAnonymous?.wakif_province || "12",
      wakif_city: donationAnonymous?.wakif_city || "12.71",
  ...
  const onSubmit = async (values: z.infer<typeof FormSchema>) => {
    try {
      const payload = {
        ...
        wakif_city: "12.71", // KOTA MEDAN
        wakif_province: "12", // PROVINSI SUMATERA UTARA
      ...
      if (Number.isNaN(donationAnonymous?.amount) || Number(donationAnonymous?.amount) < minNominal) {
        setErrorMinWakaf(true);
        notifyError(`Nominal wakaf minimal Rp ${toMoney(minNominal)}`);
        return;
      }
        ...
        .then((response) => {
          ...
          setTimeout(() => {
            if (payment?.name === "QRIS") {
              router.push(`/campaign/${params.campaign_id}/anonymous/${id}?type=qris`);
            } else {
              router.push(`/campaign/${params.campaign_id}/anonymous/${id}?type=va`);
            }
          }, 800);
          setLoading(false);
          setOpenIkrar(false);
  ...
  return (
    <SumutLayout footer="detail-page">
      <section className="pt-4 pb-8 w-full relative z-10 space-y-3 max-w-7xl mx-auto px-3">
        <button
          type="button"
          className="flex items-center gap-2"
          onClick={() => router.push(`/campaign/${params?.campaign_id}/detail/about`)}
        >
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
            <div className="grid grid-cols-12 gap-4">
              <div
                className="col-span-full md:col-span-7 rounded-2xl p-4 border-gray-50 border-2 backdrop-blur-xl"
                style={{
              ...
              >
                ...
                {isLoading ? (
                  ...
                  <section className="px-2 md:px-5 pt-2 pb-3">
                ...
                {isLoading ? (
                  ...
                  <div className="px-2 md:px-5 mt-4 space-y-4">
                    ...
                    {/* PEMBAYAR */}
                    <div>
                      <h1 className="text-sm font-semibold text-white my-3">Pembayar</h1>
                    ...
                    <div>
                      ...
                      <div className="space-y-4">
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel className="text-white">Doa & Harapan (Opsional)</FormLabel>
                              <FormControl>
                                <Textarea
                                  {...field}
                                  placeholder="Tuliskan doa & harapan anda"
                                  disabled={isSubmitting}
                                  className="bg-gray-50/20 text-white font-medium placeholder:text-gray-100 min-h-[75px] text-xs"
                                />
                              </FormControl>
                              <FormMessage className="!text-red-600 !text-xs font-medium tracking-wider" />
                            </FormItem>
              ...
              <div className="col-span-full md:col-span-5 backdrop-blur-xl h-fit">
                <div
                ...
                >
                  ...
                  <Separator />
                  <PaymentMethods
                    selectPayment={(payment) => handleSelectPaymentMethod(payment)}
