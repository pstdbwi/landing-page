...
import LayoutMurabi from "@/app/murabbi/_components/murabi-layout";
import { MurabiScreenLoading } from "@/app/murabbi/_components/murabi-screen-loading";
...
import { ChevronDownIcon, CopyIcon, DownloadIcon, FileTextIcon, FlagIcon, Share2Icon } from "lucide-react";
...
- Total Pembayaran: ${detail?.amount?.total_amount}`;
  ...
  const shareToWhatsapp = () => {
    const message = `Gunakan tautan berikut apabila Anda ingin kembali ke halaman Pembayaran Wakaf:\n${fullURL}`;
    const whatsappUrl = `https://wa.me/?text=${encodeURIComponent(message)}`;
    window.open(whatsappUrl, "_blank");
  };

  return (
    <LayoutMurabi footer="detail-page">
        ...
        >
          ...
          {detail?.status?.name == "VERIFIED" && (
            <Fragment>
              ...
              <div className="w-full flex items-center gap-1">
                <Link
                  href={`/campaign/${params?.campaign_id}/anonymous/${params?.id}/receipt`}
                  target="_blank"
                  className={buttonVariants({
                    variant: "outline",
                    className:
                      "font-semibold bg-fesyar-gold hover:bg-fesyar-yellow-600 w-full text-fesyar-green-700 border-fesyar-gold hover:border-fesyar-yellow-600 py-3 rounded-xl shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 transition-all duration-200",
                  })}
                >
          ...
          {detail?.status?.name == "PENDING" && (
            <div className="flex justify-between items-start md:items-center gap-4">
              <p className="text-[0.65rem] lg:text-sm text-white font-medium italic">
                *Untuk dapat kembali mengakses halaman ini, silakan bagikan (share) halaman ini terlebih dahulu.
              </p>
              <button
                type="button"
                className="text-white text-sm font-semibold underline flex items-center"
              ...
              >
                Bagikan
                <Share2Icon className="w-4 ml-2 text-white" />
              </button>
            </div>
          )}
