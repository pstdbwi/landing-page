...
import { z } from "zod";
import MurabiPaymentMethods from "./murabi-payment-methods";
import { MurabiScreenLoading } from "./murabi-screen-loading";
...
const MurabiAuthDonation = ({ authModal, setAuthModal, campaign }: Props) => {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const payload = {
        ...
        email: values?.email || "murabbi.satuwakaf.id",
        ...
        subdomain: "murabbi.satuwakaf.id",
  ...
  const defaultQrisMethod: IPaymentMethod = campaign?.payment_methods?.instant_payment?.[0] ?? FALLBACK_QRIS;
  ...
  return (
    <Fragment>
      ...
      >
        <DialogContent
          className="border-none rounded-xl w-full max-w-lg max-h-[90vh] overflow-hidden [&_[data-slot=dialog-close]]:text-white [&_[data-slot=dialog-close]]:opacity-100"
          style={{
        ...
        >
          ...
          <Image
            ...
            height={627}
            alt="ornament"
            className="pointer-events-none fixed bottom-0 left-0 z-0 h-[45vh] max-h-[627px] w-auto"
          />
          <Image
            src="/assets/murobbi/ornament-bottom-left-right.png"
            width={113}
            height={627}
            alt="ornament"
            className="pointer-events-none fixed bottom-0 right-0 z-0 h-[45vh] max-h-[627px] w-auto scale-x-[-1]"
          />

          <div className="relative z-40 max-h-[calc(90vh-5rem)] overflow-y-auto pr-1">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 relative">
                      ...
                      <FormItem>
                        ...
                        <ReactSelectFesyar
                          ...
                          onChange={(val: any) => {
                            ...
                            form.trigger(["corp_unit_group", "corp_unit_lvl1_name"]);
                          }}
                ...
                <div className="pt-4 border-t border-white/20 space-y-2 text-justify">
                  ...
                  <p className="text-[10px] text-gray-200 leading-tight">
                    <span className="font-bold">2. Komitmen:</span> SatuWakaf berkomitmen penuh bahwa segala data
                    terkait pribadi yang dikumpulkan akan dikelola dan dijaga dengan tata kelola data sesuai aturan
                    sebaik-baiknya dari akses ilegal, preventif kebocoran data, atau penyalahgunaan
                  </p>
                </div>
      ...
      {screenLoading ? <MurabiScreenLoading /> : null}
    </Fragment>
  );
};

export default MurabiAuthDonation;
