...
import EposCampaignBanner from "@/components/shared-program/epos/epos-campaign-banner";
import EposCampaignNavigation from "@/components/shared-program/epos/epos-campaign-navigation";
import EposDesktopSideCard from "@/components/shared-program/epos/epos-desktop-side-card";
import LayoutEpos from "@/components/shared-program/epos/epos-layout";
import EposMobileBottomCta from "@/components/shared-program/epos/epos-mobile-bottom-cta";
import EposMobileCampaignInfo from "@/components/shared-program/epos/epos-mobile-campaign-info";
import EposAuthDonation from "@/components/shared-program/epos/epos-auth-donation";
...
const Layout = ({ children, params }: { children: React.ReactNode; params: { campaign_id: string } }) => {
  ...
  return (
    <LayoutEpos footer="detail-page" tnc={true}>
      {/* Desktop Side Card */}
      <EposDesktopSideCard
        campaign={campaign}
        ...
        <div className="flex max-md:flex-col justify-center w-full items-start md:gap-2 lg:gap-5">
          {isCampaignLoading ? (
            ...
            <>
              ...
              />
              <EposMobileCampaignInfo campaign={campaign} percent={percent} donorCount={donorCount} />
            </>
          )}
      ...
      {/* Navigation Tabs */}
      <EposCampaignNavigation campaignId={campaignId} donorCount={donorCount} />
      ...
      {/* Mobile Bottom CTA */}
      <EposMobileBottomCta campaignType={campaign?.type} onDonate={handleDonate} isDisabled={false} />

      {/* Dialogs */}
      <EposAuthDonation authModal={authModal} setAuthModal={setAuthModal} campaign={campaign} />
