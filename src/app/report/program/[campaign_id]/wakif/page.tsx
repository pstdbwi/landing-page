...
import currencyFormater, { NameOfPayment, qs, sanitizeTitle } from "@/lib/utils";
import { useGetCampaignDetail } from "@/services/campaign/hooks";
import { useGetReportProgramDonationHistories } from "@/services/report/hooks";
...
const ListWakifCampaignProgram = () => {
  const { campaign_id = "" } = useParams();
  const { payment_date_start = "", payment_date_end = "", program_name = "" } = useSearchParamsEntries();
  ...
  const {
    ...
    isLoading,
    isFetched,
  } = useGetReportProgramDonationHistories({
    queryString: qs({
      ...filters,
    ...
    <LayoutReport>
      <div className="py-4">
        <div className="flex items-center justify-between">
          <h1 className="text-lg md:text-xl font-bold">Rekap dan Detils {campaign?.title}</h1>
        ...
        <div className="space-y-4">
            ...
            <div className="bg-primary-800 w-full rounded-md shadow text-white flex flex-col md:flex-row items-start gap-4 md:gap-8 p-6">
              <div className="flex flex-col gap-1">
                ...
                <div className="flex items-baseline gap-1">
                  <span className="text-lg font-bold">{program_name == "null" ? "Lainnya" : program_name}</span>
                </div>
              </div>
              <div className="flex flex-col gap-1">
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              <Link
                href={`/report/program/${campaign_id}/`}
...
export default ListWakifCampaignProgram;
