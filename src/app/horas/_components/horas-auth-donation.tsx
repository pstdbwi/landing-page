...
import { z } from "zod";
import HorasPaymentMethods from "./horas-payment-methods";
const DEFAULT_MINIMUM_NOMINAL = 0;
...
const formSchema = z
  .object({
    ...
    reward_wanted: z.boolean().default(false),
...
const HorasAuthDonation = ({ authModal, setAuthModal, campaign }: Props) => {
  ...
  const rewardOptions = useMemo(() => {
    return activeProgram?.rewards?.map((item) => ({ value: item.id, label: item.name })) || [];
  }, [activeProgram?.rewards]);
  ...
  const handleCurrencyInput = (value: string) => {
    ...
    updateDonationAnonymous({
      amount: amount,
      ...(amount <= 10_000_000 && defaultQrisMethod
        ? {
            payment_method_id: defaultQrisMethod.id,
            paymentMethod: defaultQrisMethod,
          }
        : {
            payment_method_id: "",
            paymentMethod: null,
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      const payload = {
        ...
        email: values?.email || "program.wakafsumutberkah@satuwakaf.id",
        ...
        wakif_phone: values?.is_wakif_others
          ? values?.wakif_phone
            ...
            : null
          : null,
        reward_wanted: values?.reward_wanted ? 1 : 0,
        reward_id: values?.reward_wanted ? values?.reward_id : null,
        reward_name: values?.reward_wanted ? values?.reward_name : null,
        wakif_address: null,
        wakif_city: null,
        wakif_province: null,
        wakif_type: null,
        ...
        subdomain: "program.wakafsumutberkah.id",
  ...
  const FALLBACK_QRIS: IPaymentMethod = {
  ...
  const defaultQrisMethod: IPaymentMethod = resolvedPaymentMethods?.instant_payment?.[0] ?? FALLBACK_QRIS;
        ...
        <DialogContent className="bg-[radial-gradient(circle_at_center,_theme(colors.fesyar.green.300),#00484C)] border-none rounded-xl w-full max-w-lg max-h-[90vh] overflow-auto">
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 z-40 relative">
              ...
              <div className="flex items-center w-full gap-2">
                <Switch
                  ...
                  onCheckedChange={(checked) => {
                    setShowPray(checked);
                    if (!checked) {
                      form.setValue("wakif_pray", "");
                    }
                  }}
                  type="button"
                />
                <FormLabel className="text-[0.65rem] text-gray-200 italic">Tulis Doa & Harapan?</FormLabel>
              </div>
              ...
              {activeProgram?.rewards && activeProgram.rewards.length > 0 && (
                <>
                  <div className="flex items-center w-full gap-2">
                    <Switch
                      checked={form.watch("reward_wanted")}
                      onCheckedChange={(checked) => {
                        form.setValue("reward_wanted", checked);
                        if (!checked) {
                          form.setValue("reward_id", "");
                          form.setValue("reward_name", "");
                        }
                      }}
                      type="button"
                    />
                    <FormLabel className="text-[0.65rem] text-gray-200 italic">
                      Apakah Anda ingin mendapatkan hadiah?
                    </FormLabel>
                  </div>

                  {form.watch("reward_wanted") && (
                    <FormField
                      control={form.control}
                      name="reward_id"
                      render={({ field }) => (
                        <FormItem>
                          <FormLabel className="text-white text-xs">Pilih Hadiah</FormLabel>
                          <FormControl>
                            <ReactSelectFesyar
                              name="reward_id"
                              ...
                              value={rewardOptions.find((item) => item.value === field.value)}
                              onChange={(option: any) => {
                                field.onChange(option?.value || "");
                                form.setValue("reward_name", option?.label || "");
                              }}
                              className="text-xs"
                            />
                          </FormControl>
              ...
              <HorasPaymentMethods
                ...
                defaultValue={defaultQrisMethod?.id}
...
export default HorasAuthDonation;
