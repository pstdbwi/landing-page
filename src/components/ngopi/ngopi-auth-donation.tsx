...
import { NGOPI_DEFAULT_QRIS_PAYMENT_METHOD, NGOPI_QRIS_MAX_AMOUNT } from "./constants";
import NgopiPaymentMethods from "./ngopi-payment-methods";
import { ScreenLoaderNgopi } from "./screen-loader-ngopi";
...
const NgopiAuthDonation = ({ authModal, setAuthModal, campaign }: Props) => {
  ...
  const minNominal =
    activeProgram?.min_amount && activeProgram.min_amount > 0 ? activeProgram.min_amount : DEFAULT_MINIMUM_NOMINAL;
  const donationAmount = Number(donationAnonymous?.amount || 0);
  const selectedPaymentMethod = donationAnonymous?.paymentMethod;
  const isDonationAmountValid = donationAmount >= minNominal;
  const isSelectedPaymentAllowed =
    !!donationAnonymous?.payment_method_id &&
    !(donationAmount > NGOPI_QRIS_MAX_AMOUNT && selectedPaymentMethod?.type === "instant_payment");
  const isNameValid = !!form.watch("name")?.trim();
  const isWakifNameValid = !form.watch("is_wakif_others") || !!form.watch("wakif_name")?.trim();
  const isSubmitDisabled =
    isLoading || !eula || !isDonationAmountValid || !isNameValid || !isWakifNameValid || !isSelectedPaymentAllowed;

  const defaultQrisMethod: IPaymentMethod =
    resolvedPaymentMethods?.instant_payment?.[0] ?? NGOPI_DEFAULT_QRIS_PAYMENT_METHOD;
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
      ...
      if (
        Number(donationAnonymous?.amount || 0) > NGOPI_QRIS_MAX_AMOUNT &&
        donationAnonymous?.paymentMethod?.type === "instant_payment"
      ) {
        setErrorPaymentMethod(true);
        setManualError("Nominal di atas Rp 20.000.000 harus menggunakan metode pembayaran VA");
        return;
      }
      ...
      const payload = {
        ...
        email: values?.email || window.location.host,
...
export default NgopiAuthDonation;
