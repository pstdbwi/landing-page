...
const CampaignCard = React.forwardRef<HTMLButtonElement, CardProps>(
  (
    {
      className,
      title,
          ...
          {youtubeId && isPlaying ? (
            ...
            <>
              <Image
                ...
                priority
                className={cn(
                  "mt-0 object-cover transition-transform duration-300 ease-in-out group-hover:scale-105",
        ...
        <div className={cn("space-y-1 p-1", variant === "vertical" ? "p-2" : "w-full")}>
          {location ? (
            <div className="inline-flex items-center space-x-2">
              ...
              <span className="text-gray-500 text-[0.625rem] line-clamp-1">
          ...
          <div className="inline-flex space-x-1">
            <h2 className="text-xs text-gray-500 line-clamp-1">{campaigner.name}</h2>
            <Verivied />
            <IcOrg />
          </div>
          <Progress value={percent} />
          <div className="space-y-1">
            <p className="text-base font-semibold text-primary-500">Rp {currencyFormater(donationAmount)}</p>
          </div>

          {!expired ? (
            <div className="inline-flex justify-between items-center w-full">
              <p className="text-[0.625rem] text-gray-500">Wakaf terkumpul</p>
            ...
            <div className="inline-flex justify-between items-center w-full">
              ...
              <div className="inline-flex items-center gap-1">
                ...
                <span className="text-[0.625rem] text-gray-500">{getRemainingDays(expired)} hari lagi`</span>
              </div>
            </div>
          )}
        </div>
...
export { CampaignCard, cardVariants };
